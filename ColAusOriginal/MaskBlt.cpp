...
)
{
	...
	int static flood_fill = 0;

	if (flood_fill == 0)
		if ((GetDeviceCaps(hdcDest, RASTERCAPS) & RC_FLOODFILL))
			flood_fill = 1;
		else 
			flood_fill = 2;
	...
	if (flood_fill == 1)
	//if ((GetDeviceCaps(hdcDest, RASTERCAPS) & RC_FLOODFILL)) 
        return MaskBlt(hdcDest, nXDest, nYDest, 
					   nWidth, nHeight, 
					   hdcSrc, nXSrc, nYSrc, 
					   hbmMask, xMask, yMask,
					   dwRop);
...
{
	//display the last error reported by windows
	LPVOID lpMsgBuf;
...
//extern ofstream ColAusLog; //("Colmap Event Log.txt", ios::out);
